...
.filter(filtrarMaximo).filter(filtarColor).filter(filtrarPuertas).filter(filtrarTransmision);
...
return auto.precio >= parseFloat(datosBusqueda.minimo); 
}
return auto;
}
...
return auto.precio <= parseFloat(datosBusqueda.maximo); 
}
return auto;
}
